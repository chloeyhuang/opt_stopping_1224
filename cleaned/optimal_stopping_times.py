...
#   generates partial signatures from trade data; pos data is used only as time index / OU sample generation
def gen_partial_sigs(trade_data, time, N, M, depth, interval = 1, emp = True, v = False, b_cor = 1):
    paths, time_index = gen_OU_sample(trade_data, time, N = N, M = M, emp = emp, interval = interval, log = True)
...
#   final params decided were M = 100, depth = 2, k = 1, interval = 10/20, (N = 600/900)
def get_opt_stopping_time_batched(trade_data, time, N, M, depth, k = 0.4, interval = 1, emp=False, v= False, batches = [15, 2]):
    ticker = tqdm(range(batches[0])) if v==True else range(batches[0])
    res = [gen_partial_sigs(trade_data, time, N, M, depth, interval, emp) for i in ticker]
    ...
    def batch(i):
        ...
        for i in range(batches[1]):
            ...
            #   messing around with matrices to get them in desired form
            in_prod = ps @ l_opt
            sig_dist = in_prod**2
    ...
    return st_nz[losses_min] #  returns the batches[1] best optimal stopping times